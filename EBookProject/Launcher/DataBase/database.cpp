...
const QString SELECTLAST2RECORDSFROMTOUCHEDTABLE = "select * from touchedbooklist order by xid desc limit 0,2";
...
QList<localDirectoryItem> *Database::getLastTwoRecordsFromTouchedTable()
{
    QList<localDirectoryItem> *templist = new QList<localDirectoryItem>;
    QSqlQuery query(database);
    localDirectoryItem item;
    query.prepare(SELECTLAST2RECORDSFROMTOUCHEDTABLE);
    if(query.exec()){
        while(query.next()){
            item.xid = query.value(0).toInt();
            item.file_name = query.value(1).toString();
